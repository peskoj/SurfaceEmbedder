...
begin
  ...
  for j:=2 to n do { po vseh parih tock }
    ...
    for i:=m+1 to maxPvz do
      begin pvz[i][_T] := 0;  pvz[i][_F] := 0; end;
...
begin
  ...
  for i:=1 to n do begin
    ...
    for j:=1 to stop[i] do begin
      k:=tocke[i][j];
      write(izhod,pvz[k][pvz[k][_F]=i]:3);
    end;
    writeln(izhod,'  0');
  end;
  writeln(izhod);
  ...
  for i:=1 to n-1 do
    for j:=1 to stop[i]-1 do
      for k:=j+1 to stop[i] do begin
        f := tocke[i][j];  v1 := pvz[f][_T]+pvz[f][_F]-i;
        f := tocke[i][k];  v2 := pvz[f][_T]+pvz[f][_F]-i;
...
begin
  ...
  for j:=1 to n do
    for k:=1 to stop[j] do
      if pvz[vlozG[j][k]][_T]=j then
        polpvz[vlozG[j][k]][_T] := k
      else
        polpvz[vlozG[j][k]][_F] := k;

  { Izracunaj zacetno Eulerjevo karakteristiko }
  { poisci stevilo lic }
  for l:=1 to m do { vse pvz nepregledane }
    begin pregled[l][_T] := _F; pregled[l][_F] := _F; end;
  stLic := 0;
...
begin { glavni prg }
      ...
      if rod<2 then
        ...
        if rod=2 then begin { Minimalna ovira ima rod 2. }
          ...
          for i:=1 to m do begin
            ...
            Preimenuj(pvz[m+1][_T],m+1,i);
            Preimenuj(pvz[m+1][_F],m+1,i);

            if RodGrafa>1 then ovira := false;

            { Vrnemo povezavo i: }
            Preimenuj(pvz[m+1][_T],i,m+1);
            Preimenuj(pvz[m+1][_F],i,m+1);
            m := m+1;
            pvz[i] := e;
            stop[v1] := stop[v1]+1;  stop[v2] := stop[v2]+1;
            sosedi[v1] := sosedi[v1]+[v2];
  ...
  writeln;
  HighVideo; writeln(' n = ',n:0); LowVideo;
